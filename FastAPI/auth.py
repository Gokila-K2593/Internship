...
def signup(form_data: OAuth2PasswordRequestForm = Depends(),session: Session = Depends(get_session)):
    user_exists = session.exec(
        select(Auth).where(Auth.username == form_data.username)
    ).first()
    if user_exists:
        raise HTTPException(status_code=400, detail="Username already exists")
    new_user = Auth(username=form_data.username, password=form_data.password)
...
def login(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    user = session.exec(
       select(Auth).where(
    (Auth.username == form_data.username) & (Auth.password == form_data.password)
)
...
def protected(api_key: str = Security(api_key_scheme)):
    ...
    return {"message": f"Welcome {user_data['sub']}! You are logged in!"}
